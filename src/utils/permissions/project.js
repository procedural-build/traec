...
export const projectPermissionCheck = function(projectId, requiresAdmin, requiredActions, allow_fetch=true) {
    ...
    if (!permissions) { 
        if (projectId && allow_fetch) {fetchProjectUserPermissions(projectId)}
        return null 
    }
...
const permisionOjectWarning = (<p>No user permissions found for this Project.  Please contact the project admin to ensure you have permissions to view this Project</p>)

export const projectPermissionRender = function(projectId, requiresAdmin, requiredActions, renderContent, showWarning=false) {
