...
const edgeSetFunctions = {
    treetree: (commitId, edge, newState) => {
        ...
        if (parent) {
            newState = newState.addListToSets(
                [`commitEdges.byId.${commitId}.trees.${parent.uid}.trees`],
                [child.uid]
            )
    ...
    treedocument: (commitId, edge, newState) => {
        ...
        newState = newState.addListToSets(
            [`commitEdges.byId.${commitId}.trees.${tree.uid}.documents`],
            [document.uid]
        )
        return newState
    },
    ...
    treescore: (commitId, edge, newState) => {
        ...
        newState = newState.addListToSets(
            [`commitEdges.byId.${commitId}.trees.${tree.uid}.metricScores`],
            [score.uid]
        )
        return newState
    },
    ...
    documentstatus: (commitId, edge, newState) => {
        ...
        newState = newState.setInPath(
            `commitEdges.byId.${commitId}.documents.${document.uid}.status`,
            status.uid
        )
        return newState
    },
    documentscore: (commitId, edge, newState) => {
        ...
        newState = newState.addListToSets(
            [`commitEdges.byId.${commitId}.documents.${document.uid}.scores`],
            [score.uid]
        )
        return newState
    },
