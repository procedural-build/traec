...
export const postRootRef= ({trackerId}) => {
    ...
    const stateSetFunc= (state, action) => {
        ...
        if (!data.errors) { 
            ...
            newState = newState.setInPath(`trackers.byId.${trackerId}.alt_root_masters.${data.latest_commit.root_commit}`, data.uid)
        }
        return newState            
    }
    return {fetchParams, stateParams: {stateSetFunc}}
    ...
    const stateSetFunc= (state, action) => {
        ...
        for (let item of data) {
            ...
            commitId = item.commit ? 
                item.commit : 
                targetCommit ? targetCommit.uid : null || targetRef.latest_commit.uid
            branchId = item.branchId
            let rootPath = isRoot ? 
                `root.${commitId}.branch.${branchId}.byId` : 
                `commit.${commitId}.branch.${branchId}.byId`
